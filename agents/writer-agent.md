...
9. **T3 cierra con liberación del equipo descubierta para ESTE prospecto.** Después de declarar producto + compresión temporal, el último elemento de T3 debe responder dos preguntas en una línea natural: (a) ¿de qué trabajo manual concreto se libera el equipo del prospecto? (b) ¿en qué análisis de alto valor de su rol/realidad se enfoca ahora? **No hay phrasing canónico** — los ejemplos en `outreach/references/outreach-rules.md` (lending: "análisis de riesgo y compliance", manufactura: "análisis de costos y proveedores", etc.) son **inspiración de dirección, no template**. Tu trabajo en cada iteración del loop es ENCONTRAR la frase específica que aplique únicamente a este prospecto basándote en su ficha. Si T3 termina en "imagina preguntar X y tener Y" (superficie), "se enfoca en lo que importa" (genérico), o un cierre verbatim de los ejemplos (template), reescribe
